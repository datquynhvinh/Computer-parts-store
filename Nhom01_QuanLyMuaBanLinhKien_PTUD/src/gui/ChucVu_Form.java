...
//                    JOptionPane.showMessageDialog(null,"Tên chức vụ không được rỗng !");
//                }else {
//                	if(txtTen.getText().trim().matches("^[\\p{L} ]{3,50}+$")) {
                		
            	if(CheckingMethod.checkTen(txtTen.getText().trim())==false){
                    JOptionPane.showMessageDialog(null,"Tên chức vụ không hợp lệ !");
                }else {
                int r = table.getSelectedRow();
                ChucVu cvSua = new ChucVu(txtMa.getText(),txtTen.getText());
                if(r != -1){
                    int lc = JOptionPane.showConfirmDialog(null,"Bạn chắc chắn muốn sử thông tin?","Update",JOptionPane.YES_NO_OPTION);
                    if(lc == JOptionPane.YES_OPTION){
                        if(cvDao.updateChucVu(table.getValueAt(r,1).toString(),cvSua)){
...
//                	
            	if(CheckingMethod.checkTen(txtTen.getText().trim())==false){
                    JOptionPane.showMessageDialog(null,"Tên chức vụ không hợp lệ !");
                }else {
                boolean rs = true;
