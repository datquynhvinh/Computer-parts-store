...
//        ntn.setPreferredSize(new Dimension(150,40));
        ...
        datHang.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                pn_Center.validate();
                     ...
                     JOptionPane.showMessageDialog(null,"Nhân viên không thể đặt hàng !");
                 }
            }
        });
        ...
        nhapHang.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
            	if (nv != null ) {
                NhapHang_Form nh = new NhapHang_Form();
                nh.nv = nv;
                nh.nh = nh;
                nh.doShow();
                pn_west.setVisible(false);
        ...
        htk.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					Connection con = DriverManager.getConnection("jdbc:jtds:sqlserver://localhost:1433/QuanLyCuaHangLinhKien;instance=SQLEXPRESS;user=sa;password=1234");
					String sql = "SELECT * FROM LINHKIEN";
					JasperDesign jdesign = JRXmlLoader.load("src\\BaoCao_Jasper\\ThongKeHangKho.jrxml");
...
//					Connection con = MyConnection.getInstance().getConnection();
					...
					JasperDesign jdesign = JRXmlLoader.load("src\\BaoCao_Jasper\\ThongKeHangBan.jrxml");
					JRDesignQuery updateQuery = new JRDesignQuery();
					updateQuery.setText(sql);
					jdesign.setQuery(updateQuery);
...
//					Connection con = MyConnection.getInstance().getConnection();
					...
					JasperDesign jdesign = JRXmlLoader.load("src\\BaoCao_Jasper\\ThongKeHangBan1.jrxml");
					JRDesignQuery updateQuery = new JRDesignQuery();
					updateQuery.setText(sql);
					jdesign.setQuery(updateQuery);
...
//					Connection con = MyConnection.getInstance().getConnection();
					...
					JasperDesign jdesign = JRXmlLoader.load("src\\BaoCao_Jasper\\ThongKeHangBan2.jrxml");
					JRDesignQuery updateQuery = new JRDesignQuery();
					updateQuery.setText(sql);
					jdesign.setQuery(updateQuery);
...
//					Connection con = MyConnection.getInstance().getConnection();
					String sql = "SELECT lk.MALK,lk.TENLK,llk.TENLOAI,nsx.TENNHASX,lk.BAOHANH,lk.DONVITINH,lk.DONGIA,ct.SOLUONG\r\n"
							...
							+ "JOIN [dbo].[NhaSanXuat] nsx ON nsx.MANHASX = lk.MANHASX\r\n"
							+ "WHERE MONTH(hd.NGAYLAPHD) = MONTH(GETDATE())";
					JasperDesign jdesign = JRXmlLoader.load("src\\BaoCao_Jasper\\ThongKeHangNhap1.jrxml");
					JRDesignQuery updateQuery = new JRDesignQuery();
					updateQuery.setText(sql);
					jdesign.setQuery(updateQuery);
...
//					Connection con = MyConnection.getInstance().getConnection();
					String sql = "SELECT lk.MALK,lk.TENLK,llk.TENLOAI,nsx.TENNHASX,lk.BAOHANH,lk.DONVITINH,lk.DONGIA,ct.SOLUONG\r\n"
							...
							+ "JOIN [dbo].[NhaSanXuat] nsx ON nsx.MANHASX = lk.MANHASX\r\n"
							+ "WHERE YEAR(hd.NGAYLAPHD) = YEAR(GETDATE())";
					JasperDesign jdesign = JRXmlLoader.load("src\\BaoCao_Jasper\\ThongKeHangNhap2.jrxml");
					JRDesignQuery updateQuery = new JRDesignQuery();
					updateQuery.setText(sql);
					jdesign.setQuery(updateQuery);
        ...
        JLabel lb_south = new JLabel("Mai Phước Thịnh - Huỳnh Hoàng Huy - Lê Thạc Đạt - Nguyễn Hoàng Huy");
        JLabel lb_nhom = new JLabel("Chương trình quản lý cửa hàng mua bán linh kiện máy tính");
...
//                pn_Center.validate();
//            }else 
            	if(obj.toString().equalsIgnoreCase("Màn hình")){
